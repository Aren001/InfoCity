...
const Bank = (props) => {
	...
	const fetchS = () => {
		setLoading(true);
		fetch('https://run.mocky.io/v3/e938b09a-4a2d-4d12-a3ed-455bcbc0a7db')
			...
			.then(data => {
				...
				// console.log(data);
				setArr(data.bank);
				setObj(data.homeBank);
				setDescript(data.descriptionBank);
			})
			.catch(err => {
	...
	return (
		<div className="super_container">
			...
			<HomeOne src={obj.src} name={t('homeBank.name')} />
												...
												<div className="row">
													...
													<div className="col-xl-5 col-lg-6">
														<div className="blog_post_content">
															...
															<div className="pb_title"><a href="#"> {t(`bank.${item.id}.name`)} </a></div>
															<div className="pb_text" >

																<p> {t('descriptionBank.address')}: {t(`bank.${item.id}.address`)}</p>
																<p> {t('descriptionBank.WorkingTime')}:  {t(`bank.${item.id}.time`)}</p>
																<p> {t('descriptionBank.phone')}: {t(`bank.${item.id}.phone`)}</p>
